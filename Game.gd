...
# Level layouts live as pure DATA here, separate from engine logic, so we can iterate
# fast and stack up more levels later. A level is:
#   spawn:     Vector2 where the frog starts
#   ground:    [x, y, w, h] the starting floor
#   goal:      Vector2 where the fly sits
#   platforms: array of [x, y, w, h]  (ledges to grab/swing/land on; walls = tall thin ones)
# Design rules of thumb (match the validated feel — don't make it globally harder):
#   gaps ~190-260 vertical, offsets within MAX_LEN (400). Each ledge is both an anchor to
#   swing from AND a spot to land on (landing recharges your one tongue).
func _level_tower() -> Dictionary:
	...
	#   platforms : standable grab ledges (purple)
	#   perches   : wide safe rests between sections (green)
	#   anchors   : [x,y,r] grapple-ONLY knobs you can't stand on (amber rings) — commit to a point
	#   ceilings  : overhead slabs you grapple the UNDERSIDE of and swing beneath (stone)
	return {
		"spawn": Vector2(0, -40),
		"ground": [0, 80, 900, 60],
